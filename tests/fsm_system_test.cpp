...
// ============================================================
// FSMEventQueue Component
// ============================================================

TEST_CASE("FSMEventQueue::push 推送事件到隊列", "[fsm][eventqueue]") {
    entt::registry reg;
    auto entity = reg.create();

    reg.emplace<FSMEventQueue>(entity).push("TestEvent");
...
TEST_CASE("FSMEventQueue::clear 清空隊列", "[fsm][eventqueue]") {
    FSMEventQueue queue;
    queue.push("Event1");
    queue.push("Event2");
