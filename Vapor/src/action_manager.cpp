...
namespace Vapor {
    ...
    auto ActionManager::start(std::shared_ptr<Action> action, const std::string& tag) -> std::shared_ptr<Action> {
        if (!action) {
            return nullptr;
        }
    ...
    auto ActionManager::getActionsByTag(const std::string& tag) const -> std::vector<std::shared_ptr<Action>> {
        auto it = m_tagActions.find(tag);
        if (it == m_tagActions.end()) {
            return {};
        }
