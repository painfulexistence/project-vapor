...
public:
    ...
    // Configuration
    void setConfig(const PhysicsDebugConfig& config) { this->config = config; }
    PhysicsDebugConfig& getConfig() { return config; }
    const PhysicsDebugConfig& getConfig() const { return config; }
...
private:
    ...
    void drawShape(const JPH::Shape* shape, const glm::vec3& position,
