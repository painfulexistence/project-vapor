...
//   serializer.save(reg, toSave, gltfPath, outPath);
...
namespace Vapor {
    ...
    inline json toJson(const glm::quat& q) {
    ...
    public:
        ...
        SaveResult save(
            ...
            if (!file.is_open()) return { false, fmt::format("Cannot open '{}' for writing", outPath), 0 };
        ...
        void registerEngineComponents() {
            registerComponent("transform", [](json& out, entt::registry& reg, entt::entity e) {
                if (auto* t = reg.try_get<TransformComponent>(e))
                    out = { { "position", toJson(t->position) },
            ...
            registerComponent("meshRenderer", [](json& out, entt::registry& reg, entt::entity e) {
                if (auto* m = reg.try_get<MeshRendererComponent>(e)) out = { { "visible", m->visible } };
            });
        }
    };
