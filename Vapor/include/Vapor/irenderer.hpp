...
public:
    ...
    // Terrain detail layers {grass, rock, dirt, snow} (albedo + tangent-space
    // normal each), generated by TerrainTextureGen and pushed once by
    // TerrainSystem. The RHI renderer packs them into two 2D-array textures the
    // Main pass's terrain branch (MaterialShader::Terrain) samples with
    // world-space tiling; backends without that branch ignore the call. All
    // eight images must share one square resolution.
    ...
    // renders true displaced terrain detail). Metal runs the mesh/task or
    // instanced route, Vulkan the GLSL compute + instanced route. Returns 0
    // when the backend has no tessellation pipelines or creation failed;
    // callers must fall back to their mesh path.
