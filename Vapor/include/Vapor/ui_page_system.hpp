...
namespace Vapor {
    ...
    public:
        ...
        static void show(entt::registry& reg, PageID id) {
            setVisible(reg, id, true);
        }
        static void hide(entt::registry& reg, PageID id) {
            setVisible(reg, id, false);
        }
        ...
        static void push(entt::registry& reg, PageID id) {
            ...
            for (auto entity : view) {
                auto& ui = view.get<UIStateComponent>(entity);
                if (!ui.menuStack.empty()) setPageVisible(ui, ui.menuStack.back(), false);
        ...
        static void pop(entt::registry& reg) {
            ...
            for (auto entity : view) {
                ...
                if (!ui.menuStack.empty()) setPageVisible(ui, ui.menuStack.back(), true);
            }
        }
        ...
        template<typename T> static T* getPage(entt::registry& reg, PageID id) {
            auto view = reg.view<UIStateComponent>();
            for (auto entity : view) {
                auto& ui = view.get<UIStateComponent>(entity);
                auto it = ui.pages.find(id);
                if (it != ui.pages.end() && it->second.page) return static_cast<T*>(it->second.page.get());
            }
            return nullptr;
        }
    ...
    private:
        ...
        static void setPageVisible(UIStateComponent& ui, PageID id, bool visible) {
            auto it = ui.pages.find(id);
            if (it != ui.pages.end()) it->second.shouldBeVisible = visible;
        }
    };
